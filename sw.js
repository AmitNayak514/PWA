...
async function cacheFirst(req) {
  ...
  try {
    ...
    await caches.open("dynamic-pwa").then((cache) => cache.put(req, response)); // Cache without cloning
...
async function networkFirst(req) {
  try {
    ...
    console.error(
      "NetworkFirst fetch failed, no cached response:",
      req.url,
      error
    );
    throw error; // Re-throw to allow handling in the main fetch event listener
  ...
  if (event && event.data) {
    ...
    if (data.method === "pushMessage") {
      if ("showNotification" in self.registration) {
        event.waitUntil(
          self.registration.showNotification("Amit is Testing", {
            body: data.message,
          })
        );
      } else {
